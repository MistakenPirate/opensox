...
export async function validateAvatarUrl(avatarUrl: string): Promise<void> {
  ...
  if (isIP(hostname)) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "Avatar URL cannot be a direct IP address. Please use a trusted image hosting service.",
    });
  }
  ...
  if (isPrivateOrLocalIP(hostname)) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message:
        "Avatar URL cannot point to localhost or private network addresses",
    });
  }
  ...
  try {
    ...
    if ((error as Error).name === "AbortError") {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message:
          "Avatar URL validation timed out. The image may be too large or the server is unresponsive.",
      });
    }
